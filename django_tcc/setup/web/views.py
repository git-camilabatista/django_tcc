...
# Purchases
@api_view(['POST', 'GET'])
def register_get_purchase(request):
    if request.method == 'POST':
        ...
        if not user:
            return Response(
                {'detail': 'Invalid user'}, status=status.HTTP_404_NOT_FOUND
            )
        user_purchases = {
            pid: p
        ...
        return Response(
            {'detail': 'No purchases found'}, status=status.HTTP_404_NOT_FOUND
        )
...
# Payments
@api_view(['POST', 'GET'])
def register_get_payment(request):
    if request.method == 'POST':
        ...
        return Response(
            {'detail': 'No payments found'}, status=status.HTTP_404_NOT_FOUND
        )
